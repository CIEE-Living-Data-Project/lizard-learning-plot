...
```
This project is a reproducible script creating a facet plot of lizard social learning data.
...
```
I predict that lizard age will positively correlate with performing the lid-flipping task correctly on the first attempt.
...
For this project, I will install the ggplot2 package and use it to generate a faceted .png scatter plot for the data.

## Study type

**Other**. This is a project to create a visualization from existing data.
...
```
I will use ggplot2's facet_grid function to create a visualization of the lizard learning data.
...
```
No randomization is involved in this project.
...
The data has already been collected in this study.
...
```
This is a project to create a practice visualization of data from "Age-dependent social learning in a lizard" (Noble et al. 2014). This doesn't
...
```
I am not responsible for the sample size of this project.
...
```
I am not responsible for data collection for this project, as it was collected previously.
...
```
I am not planning on performing any statistical analyses, only creating a visualization.
...
```
I do not plan on transforming the data. If it appears visually that a log scale would linearize it, I might do that.
...
```
Some quality assurance steps will be taken to exclude lizards which have missing values for the variables being tested and ensure only positive values exist for age. Those will be the only steps determining eligibility for inclusion.
...
```
Only subjects with complete data for the variables being visualized will be included in the plot.
...
```
I am open to commenting on any unexpected relationships between variables, and I don't think there is much harm in this case because it is a class project.
